...
def detect_file_type(content: bytes) -> str:
    """Detects file type based on content headers (magic numbers)."""
    if content.startswith(b'%PDF'):
        return 'pdf'
    elif content.startswith(b'PK\x03\x04'): 
        return 'docx'
    # Enhanced EML detection
    try:
        text_start = content[:1000].decode('utf-8', errors='ignore').lower()
        email_indicators = ['return-path:', 'received:', 'from:', 'to:', 'subject:', 'date:', 'message-id:']
        indicator_count = sum(1 for indicator in email_indicators if indicator in text_start)
...
def extract_text_from_eml(content: bytes) -> str:
    """
    Extracts text from EML content with robust, granular error handling for
    malformed email parts and headers.
    """
    try:
        ...
        for field in ['subject', 'from', 'to', 'cc', 'date']:
            value = msg.get(field, '').strip()
            if value:
                headers.append(f"{field.title()}: {value}")
        header_text = '\n'.join(headers) + '\n\n' if headers else ''
        ...
        if msg.is_multipart():
            for part in msg.walk():
                # Check if the part is an attachment and skip it
                if "attachment" in str(part.get("Content-Disposition", "")).lower():
                    continue
                
                # Use a try-except block for each part to handle individual errors
                try:
                    ...
                    elif payload and content_type == "text/html":
                        soup = BeautifulSoup(payload.decode(part.get_content_charset() or 'utf-8', 'replace'), "html.parser")
                        body_text += soup.get_text() + "\n"
                except Exception as e:
                    logger.warning(f"Could not process a part of the multipart email: {e}")
            ...
            try:
                ...
                if payload:
                    if msg.get_content_type() == "text/plain":
                        ...
                        soup = BeautifulSoup(payload.decode(msg.get_content_charset() or 'utf-8', 'replace'), "html.parser")
                        body_text = soup.get_text()
            except Exception as e:
                logger.warning(f"Could not process non-multipart email body: {e}")

        final_text = (header_text + body_text.strip()).strip()

        # Final check to ensure we extracted something
        if not final_text:
            raise ValueError("No readable content found in the email after processing.")
            
        return final_text

    except Exception as e:
        logger.error(f"Failed to extract text from EML file: {e}")
        raise ValueError(f"Failed to extract text from EML file: {e}")
